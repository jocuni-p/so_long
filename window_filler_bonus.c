...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   window_filler_bonus.c                              :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: jocuni-p <jocuni-p@student.42barcel>       +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/01/08 15:51:52 by jocuni-p          #+#    #+#             */
/*   Updated: 2024/01/30 16:37:13 by jocuni-p         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	init_tiles(t_so_long *sl)
{
	...
	if (sl->te == NULL)
		handle_error(PRINT_ERR_15, 24, sl);
	sl->tw = MLX_XFTI(sl->mlx, "./textures/tw.xpm", &sl->px, &sl->px);
	if (sl->tw == NULL)
		handle_error(PRINT_ERR_15, 24, sl);
	sl->tw2 = MLX_XFTI(sl->mlx, "./textures/tw2.xpm", &sl->px, &sl->px);
	if (sl->tw2 == NULL)
		handle_error(PRINT_ERR_15, 24, sl);
...
static void	window_filler2(t_so_long *sl)
{
	...
	if (sl->map[sl->aux_y][sl->aux_x] == '1')
		...
		r = MLX_PITW(sl->mlx, sl->win, sl->te, sl->aux_x * 32, sl->aux_y * 32);
			...
			sl->map[sl->aux_y][sl->aux_x] == 'W')
		r = MLX_PITW(sl->mlx, sl->win, sl->tw, sl->aux_x * 32, sl->aux_y * 32);
